...
function drawWheel(chancePercent) {
    // если параметр не передан, берём из текущего сохранённого значения
...
function spinPointer(targetAngle, chance, callback) {
    ...
    function animate(time) {
        ...
        currentAngle = angle;
        drawWheel(chance); // рисуем с зафиксированным шансом
        if (progress < 1) {
...
chanceSlider.addEventListener('input', () => {
    ...
    // Если не идёт вращение, обновляем отрисовку
    if (!isSpinning) {
...
spinBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Вычисляем целевой угол для стрелки, используя зафиксированный spinChance
        ...
        // Анимируем с зафиксированным шансом
        spinPointer(finalAngle, spinChance, () => {
            ...
            betSlider.disabled = false;
            maxBetBtn.disabled = false;
            // Возвращаем отрисовку к текущему слайдеру
